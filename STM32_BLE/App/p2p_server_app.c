...
void P2P_SERVER_Notification(P2P_SERVER_NotificationEvt_t *p_Notification) {
  ...
  case P2P_SERVER_SWITCH_C_NOTIFY_ENABLED_EVT:
    ...
    P2P_SERVER_APP_Context.Switch_c_Notification_Status =
        Switch_c_NOTIFICATION_ON;
...
void P2P_SERVER_APP_LED_BUTTON_context_Init(void) {
  ...
  P2P_SERVER_APP_Context.ButtonControl.Device_Button_Selection =
      0x01; /* Device1 */
 ...
 *************************************************************/
__USED void
P2P_SERVER_Switch_c_SendNotification(void) /* Property Notification */
{
  P2P_SERVER_APP_SendInformation_t notification_on_off =
      Switch_c_NOTIFICATION_OFF;
  P2P_SERVER_Data_t p2p_server_notification_data;

  p2p_server_notification_data.p_Payload =
  ...
  a_P2P_SERVER_UpdateCharData[1] =
      P2P_SERVER_APP_Context.ButtonControl.ButtonStatus;
  /* Update notification data length */
  p2p_server_notification_data.Length =
      (p2p_server_notification_data.Length) + 2;

  if (P2P_SERVER_APP_Context.Switch_c_Notification_Status ==
      Switch_c_NOTIFICATION_ON) {
    APP_DBG_MSG("-- P2P APPLICATION SERVER : INFORM CLIENT BUTTON 1 PUSHED\n");
    notification_on_off = Switch_c_NOTIFICATION_ON;
  } else {
    APP_DBG_MSG("-- P2P APPLICATION SERVER : CAN'T INFORM CLIENT - "
  ...
  if (notification_on_off != Switch_c_NOTIFICATION_OFF &&
      P2P_SERVER_APP_Context.ConnectionHandle != 0xFFFF) {
    P2P_SERVER_NotifyValue(P2P_SERVER_SWITCH_C, &p2p_server_notification_data,
