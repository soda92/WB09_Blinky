...
static const char a_GapDeviceName[] = {
    'P', 'e', 'e', 'r', ' ', 't', 'o', ' ', 'P', 'e',
    'e', 'r', ' ', 'S', 'e', 'r', 'v', 'e', 'r'}; /* Gap Device Name */
...
uint8_t a_AdvData[] = {
    2, AD_TYPE_FLAGS,
    FLAG_BIT_LE_GENERAL_DISCOVERABLE_MODE | FLAG_BIT_BR_EDR_NOT_SUPPORTED, 26,
    AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x4C, 0x00, /* Apple CoID */
    0x02, 0x15,                                     /* iBeacon Type */
    /* UUID: 00112233-4455-6677-8899-AABBCCDDEEFF */
    0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
    0xCC, 0xDD, 0xEE, 0xFF, 0x00, 0x01, /* Major */
    0x00, 0x02,                         /* Minor */
    0xC8                                /* Tx Power (-56) */
};
 ...
 */
uint8_t scan_resp_data[] = {6,  AD_TYPE_COMPLETE_LOCAL_NAME, 'S', 'T', 'M', '3',
                            '2'};
...
static void connection_complete_event(uint8_t Status,
                                      uint16_t Connection_Handle, uint8_t Role,
...
void BLE_Init(void) {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG("  Fail   : aci_hal_write_config_data command - "
                "CONFIG_DATA_PUBADDR_OFFSET, result: 0x%02X\n",
                ret);
  } else {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_gatt_srv_profile_init command, result: 0x%02X\n", ret);
  } else {
  ...
  ret = aci_gap_profile_init(role, privacy_type, &gap_dev_name_char_handle,
  ...
  APP_DBG_MSG(
      "  Static Random Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
      bd_address[5], bd_address[4], bd_address[3], bd_address[2], bd_address[1],
      bd_address[0]);
#elif (CFG_BD_ADDRESS_TYPE == HCI_ADDR_PUBLIC)
  APP_DBG_MSG("  Public Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
              bd_address[5], bd_address[4], bd_address[3], bd_address[2],
              bd_address[1], bd_address[0]);
#else
#error "Invalid CFG_BD_ADDRESS_TYPE"
#endif

  ret = Gap_profile_set_dev_name(0, sizeof(a_GapDeviceName),
                                 (uint8_t *)a_GapDeviceName);

  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : Gap_profile_set_dev_name - Device Name, result: 0x%02X\n",
        ret);
  } else {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : Gap_profile_set_appearance - Appearance, result: 0x%02X\n",
        ret);
  } else {
  ...
  bleAppContext.BleApplicationContext_legacy.bleSecurityParam.mitm_mode =
      CFG_MITM_PROTECTION;
  bleAppContext.BleApplicationContext_legacy.bleSecurityParam
  ...
  ret = aci_gap_set_security_requirements(
      bleAppContext.BleApplicationContext_legacy.bleSecurityParam.bonding_mode,
      bleAppContext.BleApplicationContext_legacy.bleSecurityParam.mitm_mode,
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG("  Fail   : aci_gap_set_security_requirements command, result: "
                "0x%02X\n",
                ret);
  } else {
   ...
   */
  if (bleAppContext.BleApplicationContext_legacy.bleSecurityParam
          .bonding_mode) {
    ret = aci_gap_configure_filter_accept_and_resolving_list(0x01);
    if (ret != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "  Fail   : aci_gap_configure_filter_accept_and_resolving_list "
          "command, result: 0x%02X\n",
          ret);
    } else {
      APP_DBG_MSG(
          "  Success: aci_gap_configure_filter_accept_and_resolving_list "
          "command\n");
    }
  }
...
void APP_BLE_Init(void) {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG(
        "  Fail   : aci_hal_set_radio_activity_mask command, result: 0x%2X\n",
        ret);
  } else {
  ...
  HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Advertising_mgr_timer_Id,
                                    ADV_TIMEOUT_MS);
...
void BLEEVT_App_Notification(const hci_pckt *hci_pckt) {
  ...
  switch (p_event_pckt->evt) /* evt field is at same offset in hci_event_pckt
                                and hci_event_ext_pckt */
  {
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE: {
    ...
    P2P_SERVERHandleNotification.ConnectionHandle =
        p_disconnection_complete_event->Connection_Handle;
  ...
  case HCI_LE_META_EVT_CODE: {
    ...
    case HCI_LE_CONNECTION_UPDATE_COMPLETE_SUBEVT_CODE: {
      ...
      APP_DBG_MSG("     - Connection Interval:   %d.%02d ms\n     - Connection "
                  ...
                  FRACTIONAL_2DIGITS(
                      p_conn_update_complete->Connection_Interval * 1.25),
    ...
    case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE: {
      ...
      connection_complete_event(
          p_conn_complete->Status, p_conn_complete->Connection_Handle,
          p_conn_complete->Role, p_conn_complete->Peer_Address_Type,
          p_conn_complete->Peer_Address, p_conn_complete->Connection_Interval,
  ...
  case HCI_VENDOR_EVT_CODE: {
    ...
    case ACI_HAL_END_OF_RADIO_ACTIVITY_VSEVT_CODE:
      ...
      HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.SwitchOffGPIO_timer_Id,
                                        LED_ON_TIMEOUT_MS);
    ...
    case ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE: {
      ...
      ret = aci_gap_numeric_comparison_value_confirm_yesno(
          ...
          confirm_value);
      if (ret != BLE_STATUS_SUCCESS) {
        APP_DBG_MSG("==>> aci_gap_numeric_comparison_value_confirm_yesno : "
                    "Fail, reason: 0x%02X\n",
                    ret);
      } else {
        APP_DBG_MSG(
    ...
    case ACI_GATT_SRV_READ_VSEVT_CODE: {
      ...
      aci_gatt_srv_resp(p_read->Connection_Handle, p_read->CID,
                        p_read->Attribute_Handle, error_code, 0, NULL);
...
static void connection_complete_event(uint8_t Status,
  ...
  APP_DBG_MSG(
      ">>== hci_le_connection_complete_event - Connection handle: 0x%04X\n",
      Connection_Handle);
  APP_DBG_MSG(
      "     - Connection established with @:%02x:%02x:%02x:%02x:%02x:%02x\n",
      Peer_Address[5], Peer_Address[4], Peer_Address[3], Peer_Address[2],
      Peer_Address[1], Peer_Address[0]);
  APP_DBG_MSG("     - Connection Interval:   %d.%02d ms\n     - Connection "
              "latency:    %d\n     - Supervision Timeout: %d ms\n",
              INT(Connection_Interval * 1.25),
              FRACTIONAL_2DIGITS(Connection_Interval * 1.25),
              Peripheral_Latency, Supervision_Timeout * 10);
...
void APP_BLE_Procedure_Gap_General(ProcGapGeneralId_t ProcGapGeneralId) {
  ...
  case PROC_GAP_GEN_PHY_TOGGLE: {
    ...
    status = hci_le_read_phy(
        bleAppContext.BleApplicationContext_legacy.connectionHandle, &phy_tx,
        &phy_rx);
    if (status != BLE_STATUS_SUCCESS) {
      ...
      if ((phy_tx == HCI_TX_PHY_LE_2M) && (phy_rx == HCI_RX_PHY_LE_2M)) {
        ...
        status = hci_le_set_phy(
            ...
            HCI_TX_PHYS_LE_1M_PREF, HCI_RX_PHYS_LE_1M_PREF, 0);
        if (status != BLE_STATUS_SUCCESS) {
          APP_DBG_MSG("Fail\n");
        } else {
          APP_DBG_MSG("Success\n");
          gap_cmd_resp_wait(); /* waiting for
                                  HCI_LE_PHY_UPDATE_COMPLETE_SUBEVT_CODE */
        }
      } else {
        APP_DBG_MSG("==>> hci_le_set_phy PHY Param  TX= %d, RX= %d - ",
  ...
  case PROC_GATT_EXCHANGE_CONFIG: {
    status = aci_gatt_clt_exchange_config(
        bleAppContext.BleApplicationContext_legacy.connectionHandle);
    if (status != BLE_STATUS_SUCCESS) {
...
void APP_BLE_Procedure_Gap_Peripheral(
    ProcGapPeripheralId_t ProcGapPeripheralId) {
  tBleStatus status;
  ...
  case PROC_GAP_PERIPH_CONN_TERMINATE: {
    status = aci_gap_terminate(
        bleAppContext.BleApplicationContext_legacy.connectionHandle, 0x13);
    if (status != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG("aci_gap_terminate failure: reason=0x%02X\n", status);
    } else {
      APP_DBG_MSG("==>> aci_gap_terminate : Success\n");
  ...
  case PROC_GAP_PERIPH_ADVERTISE_START_LP: {
    ...
    status = aci_gap_set_advertising_configuration(
        0, GAP_MODE_GENERAL_DISCOVERABLE, ADV_TYPE, paramA, paramB,
        HCI_ADV_CH_ALL, 0, NULL, /* No peer address */
        HCI_ADV_FILTER_NONE, 0,  /* 0 dBm */
        HCI_PHY_LE_1M,           /* Primary advertising PHY */
        0,                       /* 0 skips */
        HCI_PHY_LE_1M, /* Secondary advertising PHY. Not used with legacy
                          advertising. */
        0,             /* SID */
        0 /* No scan request notifications */);
    if (status != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "==>> aci_gap_set_advertising_configuration - fail, result: 0x%02X\n",
          status);
    } else {
    ...
    status = aci_gap_set_advertising_data(
        0, ADV_COMPLETE_DATA, sizeof(a_AdvData), (uint8_t *)a_AdvData);
    if (status != BLE_STATUS_SUCCESS) {
    ...
    status = aci_gap_set_scan_response_data(0, sizeof(scan_resp_data),
                                            scan_resp_data);
    if (status != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "==>> aci_gap_set_scan_response_data Failed, result: 0x%02X\n",
          status);
    } else {
    ...
    if (status != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "==>> aci_gap_set_advertising_enable Failed, result: 0x%02X\n",
          status);
    } else {
  ...
  case PROC_GAP_PERIPH_CONN_PARAM_UPDATE: {
    status = aci_l2cap_connection_parameter_update_req(
        bleAppContext.BleApplicationContext_legacy.connectionHandle, paramA,
...
void APPE_Button1Action(void) {
  if (bleAppContext.Device_Connection_Status == APP_BLE_IDLE) {
    ...
    HAL_RADIO_TIMER_StartVirtualTimer(&bleAppContext.Advertising_mgr_timer_Id,
                                      ADV_TIMEOUT_MS);
  } else if (bleAppContext.Device_Connection_Status ==
