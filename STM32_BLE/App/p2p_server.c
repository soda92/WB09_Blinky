 ...
 */
#define P2P_SERVER_UUID                                                        \
  0x8f, 0xe5, 0xb3, 0xd5, 0x2e, 0x7f, 0x4a, 0x98, 0x2a, 0x48, 0x7a, 0xcc,      \
      0x40, 0xfe, 0x00, 0x00
#define LED_C_UUID                                                             \
  0x19, 0xed, 0x82, 0xae, 0xed, 0x21, 0x4c, 0x9d, 0x41, 0x45, 0x22, 0x8e,      \
      0x41, 0xfe, 0x00, 0x00
#define SWITCH_C_UUID                                                          \
  0x19, 0xed, 0x82, 0xae, 0xed, 0x21, 0x4c, 0x9d, 0x41, 0x45, 0x22, 0x8e,      \
      0x42, 0xfe, 0x00, 0x00

BLE_GATT_SRV_CCCD_DECLARE(switch_c, CFG_BLE_NUM_RADIO_TASKS,
...
/* p2p Server service LED_C (write without response), SWITCH_C (notification)
 * characteristics definition */
static const ble_gatt_chr_def_t p2p_server_chars[] = {
    {.properties =
         BLE_GATT_SRV_CHAR_PROP_READ | BLE_GATT_SRV_CHAR_PROP_WRITE_NO_RESP,
...
static BLEEVT_EvtAckStatus_t P2P_SERVER_EventHandler(aci_blecore_event *p_evt) {
  ...
  case ACI_GATT_SRV_ATTRIBUTE_MODIFIED_VSEVT_CODE: {
    ...
    p_attribute_modified =
        (aci_gatt_srv_attribute_modified_event_rp0 *)p_evt->data;
    notification.ConnectionHandle = p_attribute_modified->Connection_Handle;
    notification.AttributeHandle = p_attribute_modified->Attr_Handle;
    notification.DataTransfered.Length = p_attribute_modified->Attr_Data_Length;
    notification.DataTransfered.p_Payload = p_attribute_modified->Attr_Data;
    if (p_attribute_modified->Attr_Handle ==
    ...
    else if (p_attribute_modified->Attr_Handle ==
  ...
  case ACI_GATT_SRV_READ_VSEVT_CODE: {
    ...
    if (p_read->Attribute_Handle == (P2P_SERVER_Context.Led_CCharHdle +
                                     CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) {
      return_value = BLEEVT_Ack;
  ...
  case ACI_GATT_SRV_WRITE_VSEVT_CODE: {
    ...
    if (p_write->Attribute_Handle == (P2P_SERVER_Context.Led_CCharHdle +
                                      CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) {
      return_value = BLEEVT_Ack;
  ...
  case ACI_GATT_TX_POOL_AVAILABLE_VSEVT_CODE: {
    ...
    p_tx_pool_available_event =
        (aci_gatt_tx_pool_available_event_rp0 *)p_evt->data;
  ...
  case ACI_GATT_CLT_INDICATION_VSEVT_CODE: {
    ...
    status = aci_gatt_clt_confirm_indication(pr->Connection_Handle,
                                             BLE_GATT_UNENHANCED_ATT_L2CAP_CID);
    if (status != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG(
          "  Fail   : aci_gatt_confirm_indication command, result: 0x%x \n",
          status);
    } else {
...
void P2P_SERVER_Init(void) {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    APP_DBG_MSG("  Fail   : aci_gatt_srv_add_service command: p2p_Server, "
                "error code: 0x%x \n",
                ret);
  } else {
  ...
  P2P_SERVER_Context.P2p_serverSvcHdle = aci_gatt_srv_get_service_handle(
      (ble_gatt_srv_def_t *)&p2p_server_service);
  P2P_SERVER_Context.Led_CCharHdle = aci_gatt_srv_get_char_decl_handle(
      (ble_gatt_chr_def_t *)&p2p_server_chars[0]);
  P2P_SERVER_Context.Switch_CCharHdle = aci_gatt_srv_get_char_decl_handle(
      (ble_gatt_chr_def_t *)&p2p_server_chars[1]);
...
tBleStatus P2P_SERVER_UpdateValue(P2P_SERVER_CharOpcode_t CharOpcode,
  ...
  case P2P_SERVER_LED_C:
    memcpy(led_c_val_buffer, pData->p_Payload,
           MIN(pData->Length, sizeof(led_c_val_buffer)));
 ...
 * @param  ConnectionHandle: connection handle identifying the client to be
 * notified.
 *
 */
tBleStatus P2P_SERVER_NotifyValue(P2P_SERVER_CharOpcode_t CharOpcode,
  ...
  case P2P_SERVER_SWITCH_C:
    memcpy(switch_c_val_buffer, pData->p_Payload,
           MIN(pData->Length, sizeof(switch_c_val_buffer)));
    ret =
        aci_gatt_srv_notify(ConnectionHandle, BLE_GATT_UNENHANCED_ATT_L2CAP_CID,
                            P2P_SERVER_Context.Switch_CCharHdle + 1,
                            GATT_NOTIFICATION, pData->Length, /* charValueLen */
                            (uint8_t *)pData->p_Payload);
    if (ret != BLE_STATUS_SUCCESS) {
      APP_DBG_MSG("  Fail   : aci_gatt_srv_notify SWITCH_C command, error "
                  "code: 0x%2X\n",
                  ret);
    } else {
