...
/** @defgroup NVM_UpperLayer_Private_FunctionPrototypes Private Function
 * Prototypes
 * @{
 */
...
BLEPLAT_NvmStatusTypeDef BLEPLAT_NvmAdd(BLEPLAT_NvmRecordTypeDef Type,
  ...
  ret = NVMDB_AppendRecord(curr_handle_p, Type, Size, pData, ExtraSize,
                           pExtraData);
...
BLEPLAT_NvmStatusTypeDef BLEPLAT_NvmGet(BLEPLAT_NvmSeekModeTypeDef Mode,
  ...
  if (Mode == BLEPLAT_NVM_CURRENT) {
    ret =
        NVMDB_ReadCurrentRecord(curr_handle_p, Offset, pData, Size, &size_out);
  } else {
    if (Mode == BLEPLAT_NVM_FIRST) {
      NVMDB_HandleInit(db_id, curr_handle_p);
    }
    ret = NVMDB_ReadNextRecord(curr_handle_p, Type, Offset, pData, Size,
                               &size_out);
  }
...
void BLEPLAT_NvmDiscard(BLEPLAT_NvmSeekModeTypeDef Mode) {
  ...
  if (Mode == BLEPLAT_NVM_CURRENT) {
    if (curr_handle_p ==
        &device_id_db_h) // Do not allow to erase device ID data.
    {
      return;
    }
    ...
    /* Implementation that does not cause an immediate flash erase.
       ...
       ret = NVMDB_ReadNextRecord(&sec_gatt_db_h, ALL_TYPES, 0, NULL, 0,
       &size_out); if(ret == NVMDB_STATUS_OK)
