...
#define HP_PA_LEVEL                                                            \
  32 /* Fake PA level that can be reached in high power mode. */
...
#define ANY_HW_ERROR_INTERRUPT_Msk                                             \
  (BLUE_STATUSREG_ADDPOINTERROR_Msk | BLUE_STATUSREG_RXOVERFLOWERROR_Msk |     \
   BLUE_STATUSREG_TXERROR_0_Msk | BLUE_STATUSREG_TXERROR_1_Msk |               \
   BLUE_STATUSREG_TXERROR_2_Msk | BLUE_STATUSREG_TXERROR_3_Msk |               \
   BLUE_STATUSREG_TXERROR_4_Msk | BLUE_STATUSREG_ALLTABLEREADYERROR_Msk |      \
   BLUE_STATUSREG_TXDATAREADYERROR_Msk | BLUE_STATUSREG_NOACTIVELERROR_Msk |   \
   BLUE_STATUSREG_INITDELAYERROR_Msk | BLUE_STATUSREG_SEMATIMEOUTERROR_Msk |   \
   BLUE_STATUSREG_ACTIVE2ERROR_Msk | BLUE_STATUSREG_CONFIGERROR_Msk)
...
#define ANY_HW_ERROR_INTERRUPT_Msk                                             \
  (BLUE_STATUSREG_ADDPOINTERROR_Msk | BLUE_STATUSREG_RXOVERFLOWERROR_Msk |     \
   ...
   BLUE_STATUSREG_TXERROR_4_Msk | BLUE_STATUSREG_ALLTABLEREADYERROR_Msk |      \
   BLUE_STATUSREG_TXDATAREADYERROR_Msk | BLUE_STATUSREG_NOACTIVELERROR_Msk |   \
   BLUE_STATUSREG_SEMATIMEOUTERROR_Msk | BLUE_STATUSREG_ACTIVE2ERROR_Msk |     \
...
#define BLUE_TRANS_STRUCT_PTR_CAST(PTR)                                        \
  ((BlueTransStruct *)(void *)(uint8_t *)(PTR))
#endif
...
/** @defgroup BLE_Platform_Controller_Private_FunctionPrototypes Private
 * Function Prototypes
 * @{
 */
...
uint32_t BLEPLAT_CNTR_GetTimer2TimeoutForIfs(
    uint32_t T_Ifs, BLEPLAT_CNTR_Transaction Transaction, uint8_t Rx_Phy,
  ...
  if (Transaction == BLEPLAT_CNTR_RxTx) {
    ...
    if ((Rx_Phy == PHY_1MBPS) && (Tx_Phy == PHY_1MBPS)) {
      Adjust_Value = 4;
    } else if ((Rx_Phy == PHY_1MBPS) && (Tx_Phy == PHY_2MBPS)) {
      Adjust_Value = 3;
    } else if ((Rx_Phy == PHY_1MBPS) &&
               ((Tx_Phy == PHY_CODED_S2) || (Tx_Phy == PHY_CODED_S8))) {
      Adjust_Value = 4;
    } else if ((Rx_Phy == PHY_2MBPS) &&
               ((Tx_Phy == PHY_1MBPS) || (Tx_Phy == PHY_CODED_S2) ||
                (Tx_Phy == PHY_CODED_S8))) {
      Adjust_Value = 6;
    } else if ((Rx_Phy == PHY_2MBPS) && (Tx_Phy == PHY_2MBPS)) {
      Adjust_Value = 4;
    ...
    Tx_Delay_Comp =
        ((RADIO_TXDELAY_START + RADIO_TXDELAY_END) >> 3) + Adjust_Value;
  } else {
    Tx_Delay_Comp = 0;
  }

  if ((Transaction == BLEPLAT_CNTR_RxTx) ||
      (Transaction == BLEPLAT_CNTR_TxTx)) {
    if (Cal_Enabled == TRUE) {
      ...
      Init_Delay = RADIO_INITDELAY_TIMER2_NOCAL + RADIO_FSM_TX_DELAY_NO_CAL;
             ...
             (Transaction == BLEPLAT_CNTR_RxRx)) {
    /* The calculation below is based on the following sequence:
     * - When Timer2 expires the sequencer performs the 1st initialization step
     * and sends a take_req to the radio
     * - When RADIO_INITDELAY_TIMER12_CAL or RADIO_INITDELAY_TIMER2_NOCAL expire
     * the sequencer sends a tx_rx_req to the radio
     * - When the radio FSM reaches the RX state (after RADIO_FSM_RX_DELAY_CAL
     * or RADIO_FSM_RX_DELAY_NO_CAL) the demodulator is turned on
     *
     * The sum of Timer2 timeout +  TIMER(1)2_INIT_DELAY(_NO)_CAL +
     * RADIO_FSM_RX_DELAY(_NO)_CAL must be equal to the T IFS.
     *
     * The current calculation of Timer2 for TX-RX events is slightly
     * conservative since it soes not consider the delay from digital modulator
     * to antenna and from antenna to digital demodulator. As a consequence the
     * demodulator is turned on a few microseconds earlier than stricty needed.
     */
    if (Cal_Enabled == TRUE) {
...
void BLEPLAT_CNTR_TxRxSkip(void) { LL_RADIO_BlueSetTxRxSkip(0x1U); }
...
void BLEPLAT_CNTR_GlobWritePeriodslow(uint16_t x) { /* nothing to do */ }
...
uint32_t BLEPLAT_CNTR_IntGetIntStatusBitTimerOverrun(uint32_t x) { return 0; }
...
void BLEPLAT_CNTR_PacketClrCrcinitSel(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetCRCInitializationSelector((TXRXPACK_TypeDef *)packetP,
                                        (uint32_t)0x0);
}

void BLEPLAT_CNTR_PacketClrCteSamplingEn(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
...
void BLEPLAT_CNTR_PacketDisableWhitening(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetDisableWhitening((TXRXPACK_TypeDef *)packetP, (uint32_t)0x1);
}

uint8_t
BLEPLAT_CNTR_PacketGetCteSamplingEn(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
#if defined(STM32WB05) || defined(STM32WB09)
...
uint8_t *BLEPLAT_CNTR_PacketGetDataPtr(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  return (uint8_t *)CONV_ADR(
      LL_RADIO_GetDataPointer((TXRXPACK_TypeDef *)packetP));
}
...
void BLEPLAT_CNTR_PacketSetAdvPduFormat(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
...
void BLEPLAT_CNTR_PacketSetCteSamplingEn(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
#if defined(STM32WB05) || defined(STM32WB09)
...
void BLEPLAT_CNTR_PacketSetDataPduFormat(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetAdvertise((TXRXPACK_TypeDef *)packetP, (uint32_t)0x0);
}

void BLEPLAT_CNTR_PacketSetDataPtr(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP,
                                   void *dataP) {
  LL_RADIO_SetDataPointer((TXRXPACK_TypeDef *)packetP,
                          BLUE_DATA_PTR_CAST(dataP));
}
...
void BLEPLAT_CNTR_PacketSetNextPtr(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP,
                                   BLEPLAT_CNTR_TXRXPACK_TypeDef *packetNextP) {
  LL_RADIO_SetNextPointer(
      (TXRXPACK_TypeDef *)packetP,
...
void BLEPLAT_CNTR_PacketSetNextSlot(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP,
                                    uint8_t slot) {
  /* nothing to do */
}
...
void BLEPLAT_CNTR_PacketSetTimeout(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP,
                                   uint32_t x) {
  LL_RADIO_SetTimer2Triggering((TXRXPACK_TypeDef *)packetP, x);
}

void BLEPLAT_CNTR_PacketSetTimer2Active(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_Timer2_Enable((TXRXPACK_TypeDef *)packetP);
}

void BLEPLAT_CNTR_PacketSetTimerTrigDone(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetTrigDone((TXRXPACK_TypeDef *)packetP, (uint32_t)0x1);
}

void BLEPLAT_CNTR_PacketSetTimerTrigRcv(
    BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
...
void BLEPLAT_CNTR_SetRadioConfig(uint8_t *value) { /* nothing to do */ }

void BLEPLAT_CNTR_SetRcvLen(BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP,
                            uint32_t rcvLen) {
  (void)packetP;
...
uint8_t *BLEPLAT_CNTR_SmGetPrevRxPacketDataPtr(uint8_t smNo) {
  return (uint8_t *)CONV_ADR(
      BLUE_TRANS_STRUCT_PTR_CAST(LL_RADIO_GetRcvPointPrevious(smNo))->DATAPTR);
}

BLEPLAT_CNTR_TXRXPACK_TypeDef *BLEPLAT_CNTR_SmGetPrevRxPacketPtr(uint8_t smNo) {
  return (BLEPLAT_CNTR_TXRXPACK_TypeDef *)BLUE_TRANS_STRUCT_PTR_CAST(
      CONV_ADR(LL_RADIO_GetRcvPointPrevious(smNo)));
}

uint8_t *BLEPLAT_CNTR_SmGetPrevTxPacketDataPtr(uint8_t smNo) {
  return (uint8_t *)CONV_ADR(
      BLUE_TRANS_STRUCT_PTR_CAST(LL_RADIO_GetTxPointPrevious(smNo))->DATAPTR);
}

BLEPLAT_CNTR_TXRXPACK_TypeDef *BLEPLAT_CNTR_SmGetPrevTxPacketPtr(uint8_t smNo) {
  return (BLEPLAT_CNTR_TXRXPACK_TypeDef *)BLUE_TRANS_STRUCT_PTR_CAST(
      CONV_ADR(LL_RADIO_GetTxPointPrevious(smNo)));
}
...
void BLEPLAT_CNTR_SmGetRxCount(uint8_t smNo, uint32_t *packetCount) {
  ...
  packetCount[0] = packetCount[0] |
                   ((uint32_t)LL_RADIO_GetPacketCounterRx_39_24(smNo) << 24U);
...
BLEPLAT_CNTR_TXRXPACK_TypeDef *BLEPLAT_CNTR_SmGetTxPacketPtr(uint8_t smNo) {
  return (BLEPLAT_CNTR_TXRXPACK_TypeDef *)BLUE_TRANS_STRUCT_PTR_CAST(
      CONV_ADR(LL_RADIO_GetTxPoint(smNo)));
}
...
void BLEPLAT_CNTR_SmSetChannelMap(uint8_t smNo, uint8_t *chanMap) {
  ...
  value = (uint32_t)chanMap[2] | ((uint32_t)chanMap[3] << 8U) |
          ((uint32_t)chanMap[4] << 16U);
...
void BLEPLAT_CNTR_SmSetCteMaxIqsamplesNumb(uint8_t smNo,
                                           uint8_t iqsamplesNumb) {
#if defined(STM32WB05) || defined(STM32WB09)
...
void BLEPLAT_CNTR_SmSetDataLengthExtnEn(uint8_t smNo) { /* nothing to do */ }
...
void BLEPLAT_CNTR_SmSetRxCount(uint8_t smNo, uint32_t *packetCount) {
  ...
  uint32_t value =
      (uint32_t)(packetCount[1] << 8U) | (uint32_t)(packetCount[0] >> 24U);
...
void BLEPLAT_CNTR_SmSetRxMode(uint8_t smNo) { LL_RADIO_TxMode_Disable(smNo); }

void BLEPLAT_CNTR_SmSetRxPacketPtr(uint8_t smNo,
                                   BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetRcvPoint(
      smNo, (uint32_t)BLUE_STRUCT_PTR_CAST((TXRXPACK_TypeDef *)packetP));
}
...
void BLEPLAT_CNTR_SmSetTxMode(uint8_t smNo) { LL_RADIO_TxMode_Enable(smNo); }

void BLEPLAT_CNTR_SmSetTxPacketPtr(uint8_t smNo,
                                   BLEPLAT_CNTR_TXRXPACK_TypeDef *packetP) {
  LL_RADIO_SetTxPoint(
      smNo, (uint32_t)BLUE_STRUCT_PTR_CAST((TXRXPACK_TypeDef *)packetP));
}
...
void BLEPLAT_CNTR_StartEncrypt() { LL_RADIO_BlueSetManAESCmdStart(0x1U); }
