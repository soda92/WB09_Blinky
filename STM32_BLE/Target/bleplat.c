 ...
 */
void BLEPLAT_GetPartInfo(uint8_t *pDeviceId, uint8_t *pMajorCut,
                         uint8_t *pMinorCut) {
...
BLEPLAT_PkaStatusTypeDef BLEPLAT_PkaStartP256Key(const uint32_t *private_key,
                                                 BLEPLAT_PkaFuncCb funcCb) {
  return (BLEPLAT_PkaStatusTypeDef)PKAMGR_StartP256PublicKeyGeneration(
      private_key, (PKAMGR_funcCB)funcCb);
}

BLEPLAT_PkaStatusTypeDef BLEPLAT_PkaStartDHkey(uint32_t *private_key,
                                               uint32_t *public_key,
                                               BLEPLAT_PkaFuncCb funcCb) {
  return (BLEPLAT_PkaStatusTypeDef)HW_PKA_StartP256DHkeyGeneration(
      private_key, public_key, (PKAMGR_funcCB)funcCb);
}

void BLEPLAT_AesEcbEncrypt(const uint32_t *plainTextData, const uint32_t *key,
...
int32_t BLEPLAT_AesCMACEncryptAppend(BLEPLAT_AESCMACctxTypeDef *pAESCMACctx,
                                     ...
                                     int32_t InputSize) {
  return AES_CMAC_Encrypt_Append((AESCMACctx_stt *)pAESCMACctx, pInputBuffer,
                                 InputSize);
}
...
int32_t BLEPLAT_AesCMACEncryptFinish(BLEPLAT_AESCMACctxTypeDef *pAESCMACctx,
                                     ...
                                     int32_t *pOutputSize) {
  return AES_CMAC_Encrypt_Finish((AESCMACctx_stt *)pAESCMACctx, pOutputBuffer,
                                 pOutputSize);
}

void BLEPLAT_RngGetRandom16(uint16_t *num) { HW_RNG_GetRandom16(num); }

void BLEPLAT_RngGetRandom32(uint32_t *num) { HW_RNG_GetRandom32(num); }
...
uint8_t BLEPLAT_GetMaxPALevel(void) { return RADIO_GetMaxPALevel(); }

uint8_t BLEPLAT_GetDefaultPALevel(void) { return RADIO_GetDefaultPALevel(); }

void BLEPLAT_SetHighPower(uint8_t enable) {
  RADIO_SetHighPower((FunctionalState)enable);
}

int8_t BLEPLAT_CalculateRSSI(void) { return HAL_RADIO_ReadRSSI(); }

int8_t BLEPLAT_UpdateAvgRSSI(int8_t avg_rssi, int8_t rssi,
                             uint8_t rssi_filter_coeff) {
  return RADIO_UpdateAvgRSSI(avg_rssi, rssi, rssi_filter_coeff);
}

uint8_t BLEPLAT_GetDemodCI(void) { return GetDemodCI(); }
...
#if defined(STM32WB06) || defined(STM32WB07) || defined(STM32WB09)
  uint32_t dac_word =
      RRM->SYNTHCAL4_DIG_OUT & RRM_SYNTHCAL4_DIG_OUT_MOD_REF_DAC_WORD_OUT_Msk;
...
void BLEPLAT_AntIdxRemap(uint8_t AntPattLen, uint8_t *pAntRamTable,
                         const uint8_t *pAntPatt) {
...
int BLEPLAT_StartTimer(BLEPLAT_TimerHandleTypeDef *TimerHandle, uint64_t Time) {
  return HAL_RADIO_TIMER_StartVirtualTimerSysTime(
      (VTIMER_HandleType *)TimerHandle, Time);
}
...
uint8_t BLEPLAT_SetRadioTimerValue(uint32_t Time, uint8_t EventType,
                                   uint8_t CalReq) {
...
uint8_t BLEPLAT_SetRadioTimerRelativeUsValue(uint32_t RelTimeoutUs, uint8_t Tx,
                                             uint8_t PLLCal) {
