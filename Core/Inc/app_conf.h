...
#define ADV_TYPE                                                               \
  (HCI_ADV_EVENT_PROP_LEGACY | HCI_ADV_EVENT_PROP_CONNECTABLE |                \
...
/**
 * Maximum number of attributes that can be stored in the GATT database in
 * addition to the attributes number already defined for the GATT and GAP
 * services (BLE_STACK_NUM_GATT_MANDATORY_ATTRIBUTES value on STM32_BLE
 * middleware, ble_stack.h header file).
 */
#define CFG_BLE_NUM_GATT_ATTRIBUTES (6)

/**
 * Maximum number of concurrent Client's Procedures. This value must be less
 * than or equal to the maximum number of supported links
...
/**
 * Maximum number of IQ samples in the buffer used in CTE connection oriented
 * mode.
 */
...
/**
 * Maximum number of simultaneous Link Layer procedures that can be managed, in
 * addition to the minimum required by the stack. The minimum number guarantees
 * one LL procedure initiated by the peer for each link, one LL procedure
 * automatically initiated by the Controller and one LL procedure initiated by
 * the Host.
 */
...
/**
 * If 1, Peripheral Preferred Connection Parameters Characteristic is added in
 * GAP service.
 */
 ...
 */
#define CFG_BLE_MBLOCKS_COUNT                                                  \
  (BLE_STACK_MBLOCKS_CALC(CFG_BLE_ATT_MTU_MAX, CFG_BLE_NUM_RADIO_TASKS,        \
...
#define BLE_DYN_ALLOC_SIZE                                                     \
  (BLE_STACK_TOTAL_BUFFER_SIZE(                                                \
      ...
      CFG_BLE_NUM_GATT_ATTRIBUTES, CFG_BLE_NUM_OF_CONCURRENT_GATT_CLIENT_PROC, \
      CFG_BLE_MBLOCKS_COUNT, CFG_BLE_NUM_ADV_SETS, CFG_BLE_NUM_PAWR_SUBEVENTS, \
      CFG_BLE_PAWR_SUBEVENT_DATA_COUNT_MAX, CFG_BLE_NUM_AUX_SCAN_SLOTS,        \
      CFG_BLE_FILTER_ACCEPT_LIST_SIZE_LOG2, CFG_BLE_COC_NBR_MAX,               \
      CFG_BLE_NUM_SYNC_SLOTS, CFG_BLE_NUM_CTE_ANTENNA_IDS_MAX,                 \
      CFG_BLE_NUM_CTE_IQ_SAMPLES_MAX, CFG_BLE_NUM_SYNC_BIG_MAX,                \
      CFG_BLE_NUM_BRC_BIG_MAX, CFG_BLE_NUM_SYNC_BIS_MAX,                       \
      CFG_BLE_NUM_BRC_BIS_MAX, CFG_BLE_NUM_CIG_MAX, CFG_BLE_NUM_CIS_MAX,       \
      CFG_BLE_EXTRA_LL_PROCEDURE_CONTEXTS, CFG_BLE_ISR0_FIFO_SIZE,             \
      CFG_BLE_ISR1_FIFO_SIZE, CFG_BLE_USER_FIFO_SIZE))
 ...
 *  low power mode. It means that all what can have an impact on the
 * consumptions are powered down.(For instance LED, Access to Debugger, Etc.)
 *
 *  When CFG_LPM_SUPPORTED and CFG_FULL_LOW_EMULATED are both set to 1, the
 * system is configured to emulate the Deepstop mode without losing the debugger
 * connection and breakpoints nor watchpoints.
 *
 ******************************************************************************/
...
#define APP_DBG(...)                                                           \
  {                                                                            \
    UTIL_ADV_TRACE_COND_FSend(VLEVEL_L, ~0x0, ADV_TRACE_TIMESTAMP_ENABLE,      \
                              __VA_ARGS__);                                    \
  }
#else
...
/**
 * This is a bit mapping over 32bits listing all events id supported in the
 * application
 */
typedef enum {
